...
int main() {
    ...
    std::cout<<"s: "<<time1.tv_sec<<"  ms: "<<time1.tv_sec*1000+time1.tv_usec/1000<<std::endl;
    ...
    std::vector<proj_points_info> proj_points;//声明一个类型为proj_points的变量来存投影点的数据
    std::vector<xy_points_info> referenceline_xy_points;//声明一个类型为referenceline_xy_points的变量来存向前150米和向后30米的路径点
    std::vector<xy_points_info> fem_smoothing_line;//用来存平滑过后的路径点
    //画图数据记录
    ...
    for (size_t i = 0; i < 800; i++){
        ...
        std::cout<<"s: "<<time2.tv_sec-time1.tv_sec<<"  ms: "<<time2.tv_sec*1000+time2.tv_usec/1000<<std::endl;
        //------------------------------------------------------------//
        ...
        //提取referenceline 取出向前150个点 和向后30个点 减少计算量
